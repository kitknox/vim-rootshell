...
static __thread int (*dll_Gpm_Open)     (Gpm_Connect *, int);
static __thread int (*dll_Gpm_Close)    (void);
static __thread int (*dll_Gpm_GetEvent) (Gpm_Event *);
static __thread int *dll_gpm_flag;
static __thread int *dll_gpm_fd;

static __thread void *libgpm_hinst;
# endif
...
static __thread ss_pending_cmd_T *ss_pending_cmds;
...
#  endif
    static __thread struct sgttyb ttybold;
...
# else // HAVE_SELECT
	...
	static __thread fd_set	rfds, wfds, efds;
	int		maxfd;
	long		towait = msec;
...
# define STRING_INIT(s) \
    ...
    static __thread string_T sh_vimglob_func = STRING_INIT("vimglob() { while [ $# -ge 1 ]; do echo \"$1\"; shift; done }; vimglob >");
				// vimglob() function with globstar setting enabled, only for bash >= 4.X
    static __thread string_T sh_globstar_opt = STRING_INIT("[[ ${BASH_VERSINFO[0]} -ge 4 ]] && shopt -s globstar; ");
# undef STRING_INIT
...
gpm_open(void)
{
    static __thread Gpm_Connect gpm_connect; // Must it be kept till closing ?
...
{
    int			button;
    static __thread Gpm_Event	gpm_event;
    char_u		string[6];
    int_u		vim_modifiers;
    int			row,col;
    unsigned char	buttons_mask;
    unsigned char	gpm_modifiers;
    static __thread unsigned char old_buttons = 0;
...
do_xterm_trace(void)
{
    ...
    static __thread char_u	mouse_name[2] = {KS_MOUSE, KE_FILLER};
...
stop_timeout(void)
{
    static __thread struct itimerspec disarm = {{0, 0}, {0, 0}};
...
stop_timeout(void)
{
    static __thread struct itimerval disarm = {{0, 0}, {0, 0}};
    int			    ret;
