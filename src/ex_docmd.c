...
static __thread char ex_error_buf[MSG_BUF_LEN];
 ...
 */
static __thread char exmode_plus[] = "+";
    ...
    char	**errormsgp)
{
		...
		)
	{
	    ...
	    // remove it later.
	    static __thread char_u *nobslash = (char_u *)" \t\"|";
...
get_bad_name(expand_T *xp UNUSED, int idx)
{
    ...
    static __thread char *(p_bad_values[]) =
    {
	"?",
	"keep",
	"drop",
...
{
    // Note: Keep this in sync with getargopt.
    static __thread char *(p_opt_values[]) =
    {
	"fileformat=",
	"encoding=",
	"binary",
...
find_cmdline_var(char_u *src, size_t *usedlen)
{
    ...
    static __thread keyvalue_T spec_str_tab[] = {
